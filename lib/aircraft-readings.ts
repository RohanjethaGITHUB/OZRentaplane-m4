...
export function validateAircraftReadings(readings: AircraftReadings) {
  ...
  const optionalMeters: Array<[string, MaybeNumber, MaybeNumber]> = [
    ['Tacho', readings.tacho_start, readings.tacho_stop],
    ['MR', readings.mr_start, readings.mr_stop],
  ]

  for (const [label, start, stop] of optionalMeters) {
...
// Customers and admins recording post-flight readings enter only VDO and Air Switch totals.
