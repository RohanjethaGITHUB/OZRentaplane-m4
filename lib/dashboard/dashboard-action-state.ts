...
export function resolveDashboardActionState(input: DashboardActionStateInput): DashboardActionState {
  ...
  if (input.bookingFocusState?.mode === 'post_flight_clarification_required') {
    return buildState({
      ...
      responsibleActor: 'customer',
      customerActionRequired: true,
      heroLabel: 'Clarification Requested',
      heroMessage: 'Operations requested clarification on your submitted flight record.',
      actionHeading: 'Post-Flight Clarification Needed',
      actionDescription: 'Operations requested an update or clear evidence photo for your flight record. Click below to review the request and update your submission.',
      primaryAction: { label: 'Update Flight Record', href: `/dashboard/bookings/${input.bookingFocusState.bookingId}?action=flight_record` },
      secondaryAction: { label: 'View Messages', href: '/dashboard/messages' },
      nextMilestone: 'After you resubmit, operations will review the updated flight record and verify billing.',
      journeyStep: 'ready',
  ...
  if (input.bookingFocusState?.mode === 'post_flight_under_review') {
    return buildState({
      ...
      responsibleActor: 'admin',
      customerActionRequired: false,
      heroLabel: 'Flight Record Under Review',
      heroMessage: 'Your flight record has been submitted and is currently being verified by operations.',
      actionHeading: 'Your flight record is under review',
      actionDescription: 'You have submitted your flight record. Operations is currently reviewing your meter readings and billing details.',
      primaryAction: { label: 'View Booking Details', href: `/dashboard/bookings/${input.bookingFocusState.bookingId}` },
      secondaryAction: { label: 'View Messages', href: '/dashboard/messages' },
      waitingMessage: 'No action is required from you right now. Operations will verify your flight record.',
      nextMilestone: 'Once approved, your booking will be finalised and an invoice/receipt will be generated.',
      journeyStep: 'ready',
