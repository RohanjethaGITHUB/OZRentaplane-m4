...
export default function InvoiceStatusBadge({ status }: { status: PaymentStatus | string }) {
  ...
  switch (s) {
    ...
    case 'BANK_TRANSFER_PENDING_REVIEW':
      return (
        <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-300 bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-800 whitespace-nowrap">
          <span className="material-symbols-outlined text-[13px] text-amber-600 animate-pulse">hourglass_empty</span>
