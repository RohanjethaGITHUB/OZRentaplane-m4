...
export default function Navbar({ initialUser }: { initialUser: User | null }) {
  ...
  // Placing it before any hook causes a Rules of Hooks violation
  // when pathname crosses the /dashboard boundary.
    ...
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-10 h-[84px] flex items-center justify-between gap-8">
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          >
            <span
              className={`block w-6 h-0.5 bg-white/70 transition-all duration-300 origin-center ${menuOpen ? 'rotate-45 translate-y-[7px]' : ''
                }`}
            />
            <span
              className={`block w-6 h-0.5 bg-white/70 transition-all duration-300 ${menuOpen ? 'opacity-0' : ''
                }`}
            />
            <span
              className={`block w-6 h-0.5 bg-white/70 transition-all duration-300 origin-center ${menuOpen ? '-rotate-45 -translate-y-[7px]' : ''
