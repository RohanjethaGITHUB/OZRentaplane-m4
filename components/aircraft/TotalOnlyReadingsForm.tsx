...
function inputClass(compact = false, hasError = false) {
  const base = compact
    ? 'w-full border rounded-lg px-3 py-2 text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 transition-colors'
    : 'w-full border rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 transition-colors'
  return hasError
    ? `${base} bg-red-500/[0.06] border-red-500/40 focus:border-red-500/60 focus:ring-red-500/15`
    : `${base} bg-[#0a0b0d] border-white/10 focus:border-blue-500/50 focus:ring-blue-500/20`
}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {METER_LABELS.map(({ field, label }) => {
          ...
          return (
            <div key={field} className="rounded-xl border border-white/10 bg-white/[0.02] p-3 space-y-2">
