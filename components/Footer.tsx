...
export default function Footer({ forceShow = false }: FooterProps) {
  ...
  const isInstructorRoute = pathname === '/become-an-instructor' || pathname?.startsWith('/instructor') || pathname === '/dashboard/instructor'
  if (!forceShow && !isInstructorRoute && ((pathname?.startsWith('/dashboard') ?? false) || (pathname?.startsWith('/admin') ?? false))) return null
