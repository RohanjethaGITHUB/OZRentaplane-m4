-- Move terms acceptance capture from standard booking flow to checkout request flow.
-- 1) Keep standard booking RPC signature compatible, but remove terms enforcement/capture.
-- 2) Ensure acceptance table supports checkout_request records.
...
ALTER TABLE public.booking_terms_acceptances
  ADD COLUMN IF NOT EXISTS checkout_request_id uuid REFERENCES public.bookings(id) ON DELETE CASCADE,
