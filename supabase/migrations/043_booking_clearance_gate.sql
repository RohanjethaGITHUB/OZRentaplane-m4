-- ============================================================
-- 043_booking_clearance_gate.sql
--
-- Replaces the legacy verification_status gate in
-- create_aircraft_booking_atomic with a pilot_clearance_status
-- gate that matches the current checkout clearance flow.
--
-- Old check (migration 019):
--   verification_status = 'verified'
--   → raised VERIFICATION_REQUIRED
--
-- New check:
--   pilot_clearance_status = 'cleared_to_fly'
--   → raises CLEARANCE_REQUIRED
--   (mirrors the TypeScript requireClearedCustomer() guard in booking.ts)
--
-- All other logic is identical to the 019 version.
...
BEGIN
  ...
  -- The TypeScript server action (requireClearedCustomer) also enforces this,
  -- but the RPC is callable directly with any valid JWT. Checking here
  -- ensures non-cleared pilots cannot create bookings regardless of how
  -- the RPC is invoked.
