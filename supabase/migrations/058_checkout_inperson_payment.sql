-- 058_checkout_inperson_payment.sql
-- Adds support for in-person payment settlement at checkout outcome time.
...
DROP FUNCTION IF EXISTS public.complete_checkout_outcome_atomic(
  uuid, uuid, numeric, text, jsonb, text, boolean, text, integer, boolean
);
...
CREATE OR REPLACE FUNCTION public.complete_checkout_outcome_atomic(
  ...
  p_payment_received_in_person   boolean   DEFAULT false
)
RETURNS TABLE (
  out_invoice_id             uuid,
...
BEGIN
  ...
  ELSE
    ...
    IF p_payment_received_in_person THEN
      v_invoice_status       := 'paid';
      v_final_booking_status := 'completed';
      v_clearance_status     := p_checkout_outcome;
      v_total_paid_cents     := v_final_amount_cents;
  ...
  VALUES (
    ...
    v_final_amount_cents, v_advance_applied, v_amount_due, v_total_paid_cents,
  ...
  SET status         = v_final_booking_status,
      payment_status = CASE WHEN p_payment_received_in_person THEN 'paid' ELSE payment_status END,
