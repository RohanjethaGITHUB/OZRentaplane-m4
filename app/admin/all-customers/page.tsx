...
export default async function AllCustomersPage() {
  ...
  const { data: profiles, count } = await supabase
    .from('profiles')
    .select(
      'id, full_name, verification_status, updated_at, reviewed_at, admin_review_note',
      { count: 'exact' }
    )
    .eq('role', 'customer')
  ...
  return (
    <div className="p-10 max-w-7xl">
      <header className="mb-12">
        <h2 className="font-serif text-4xl font-light text-[#e2e2e6] tracking-tight">All Customers</h2>
        <p className="text-slate-400 mt-2 font-light tracking-wide">All registered customer accounts, regardless of verification status</p>
