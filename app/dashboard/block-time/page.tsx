...
export default async function BlockTimePage({
  ...
  const [{ data: profile }, { data: packageRows }, { data: purchaseRows }, { data: usageRows }] = await Promise.all([
    supabase
      .from('profiles')
  ...
  return (
    <>
      <PortalPageHero
        eyebrow="Prepaid Flying"
        title="Block Time"
        subtitle="Buy flying hours in advance at a locked-in rate, track your balance, and see every flight billed against it."
      ...
      <div className="mx-auto max-w-[1320px] space-y-8 pb-16 pt-2">
              ...
              {activePurchases.map((purchase, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {used > 0 ? (
                      <p className="mt-1.5 text-[11px] text-[#4b6390]">{used.toFixed(1)}h flown so far</p>
                    ) : null}
                  </div>
                )
              })}
              ...
              {pendingPurchases.map((purchase) => (
                ...
                >
                  ...
                  <p className="max-w-[420px] text-[12px] leading-relaxed text-amber-800">
                    Payment for this purchase was never completed. If you paid, it will activate automatically once the
                    payment is confirmed — otherwise simply start a new purchase below.
                  </p>
                </div>
              ))}
            </div>
        ...
        <section id="packages">
          ...
          <p className="mt-4 text-[12px] text-[#4b6390]">
            All packages include GST and fuel. Landing fees are charged separately. You will be sent to Stripe checkout
